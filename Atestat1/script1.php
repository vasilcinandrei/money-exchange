...
<div class="w3-bar w3-top w3-black w3-large" style="z-index:4">
  ...
  <span align = "center" class="w3-bar-item w3-left-side" >Tranzactii efectuate
</span>
</div>
...
echo '<th>Nume</th>';
    ...
    echo '<th>Valuta</th>';
    echo '<th>Suma</th></tr>';
$q1 = "SELECT Nume, CNP, Data, Tip, id_valuta, Suma
	  FROM client
...
echo '<th>Nume</th>';
    ...
    echo '<th>Valuta</th>';
    echo '<th>Suma</th></tr>';
$q2 = "SELECT Nume, CNP, Data, Tip, id_valuta, Suma
	  FROM client
	  WHERE CNP = '".$_POST['CNP']."' ";
...
while($row1= mysqli_fetch_array($result2))
{
	...
	echo '<td>'.$row1['Nume'].'</td>';
	echo '<td>'.$row1['CNP'].'</td>';
	echo'<td>'.$row1['Data'].'</td>';
    echo '<td>'.$row1['Tip'].'</td>';
    echo '<td>'.$row1['id_valuta'].'</td>';
    echo '<td>'.$row1['Suma'].'</td></tr>';
}
echo '</table>';
}
